...
for krnl in $(cat /usr/lib/modules/*/pkgbase); do
  for NVIDIA in "${krnl}-headers" "${nvidia_pkg[@]}"; do
    install_package "$NVIDIA" "$LOG_FILE"
  done
done

# Check if the Nvidia modules are already added in mkinitcpio.conf and add if not
if grep -qE '^MODULES=.*nvidia. *nvidia_modeset.*nvidia_uvm.*nvidia_drm' /etc/mkinitcpio.conf; then
  INFO "Nvidia modules already included in /etc/mkinitcpio.conf" 2>&1 | tee -a "$LOG_FILE"
else
...
printf "\n%.0s" {1..1}
INFO "Rebuilding Initramfs..." 2>&1 | tee -a "$LOG_FILE"
...
if [ -f "$NVEA" ]; then
  ...
  sudo echo -e "options nvidia_drm modeset=1 fbdev=1" | sudo tee -a /etc/modprobe.d/nvidia.conf 2>&1 | tee -a "$LOG_FILE"
  printf "\n"
fi

# Additional for GRUB users
if [ -f /etc/default/grub ]; then
    INFO "GRUB bootloader detected" 2>&1 | tee -a "$LOG_FILE"
    ...
    if ! sudo grep -q "nvidia-drm.modeset=1" /etc/default/grub; then
        ...
        OK "nvidia-drm.modeset=1 added to /etc/default/grub" 2>&1 | tee -a "$LOG_FILE"
    fi
    ...
    if ! sudo grep -q "nvidia_drm.fbdev=1" /etc/default/grub; then
        ...
        OK "nvidia_drm.fbdev=1 added to /etc/default/grub" 2>&1 | tee -a "$LOG_FILE"
    fi

    # Regenerate GRUB configuration
    if sudo grep -q "nvidia-drm.modeset=1" /etc/default/grub || sudo grep -q "nvidia_drm.fbdev=1" /etc/default/grub; then
       sudo grub-mkconfig -o /boot/grub/grub.cfg
       INFO "GRUB configuration regenerated" 2>&1 | tee -a "$LOG_FILE"
    fi

    OK "Additional steps for GRUB completed" 2>&1 | tee -a "$LOG_FILE"
fi

# Additional for systemd-boot users
if [ -f /boot/loader/loader.conf ]; then
    INFO "systemd-boot bootloader detected" 2>&1 | tee -a "$LOG_FILE"
    ...
    if [ "$backup_count" -ne "$conf_count" ]; then
        find /boot/loader/entries/ -type f -name "*.conf" | while read imgconf; do
            ...
            INFO "Backup created for systemd-boot loader: $imgconf" 2>&1 | tee -a "$LOG_FILE"
        ...
        OK "Additional steps for systemd-boot completed" 2>&1 | tee -a "$LOG_FILE"
    else
        NOTE "systemd-boot is already configured..." 2>&1 | tee -a "$LOG_FILE"
    fi
fi
