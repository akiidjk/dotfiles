...
import "/home/akiidjk/.config/quickshell/colors.js" as Colors
...
PanelWindow {
    ...
    FileView {
        ...
        preload: true
        onLoaded: {
            if (root.themeMode !== "auto")
                return;
            if (root.envTheme === "dark" || root.envTheme === "light")
                return;
    ...
    readonly property color cBgAlt: Colors.surface_container_low
    readonly property color cCard: Colors.surface_container
    readonly property color cFg: Colors.on_surface
    readonly property color cMuted: Colors.on_surface_variant
    readonly property color cBorder: Colors.outline_variant
    readonly property color cGreen: Colors.secondary
    readonly property color cRed: Colors.error
    readonly property color cBlue: Colors.primary
    ...
    function setSavedPskAndConnect(uuid, password) {
        runWithExit("nmcli connection modify uuid " + shellQuote(uuid) + " 802-11-wireless-security.key-mgmt wpa-psk " + " 802-11-wireless-security.psk " + shellQuote(password) + " && " + "nmcli -w 15 connection up uuid " + shellQuote(uuid));
    }
    ...
    function connectNew(ssid, password, username, isEnterprise) {
        ...
        let cmd = "";
        if (isEnterprise) {
            cmd = "nmcli -w 20 dev wifi connect " + shellQuote(ssid) + " password " + shellQuote(password) + " wifi-sec.key-mgmt wpa-eap " + " 802-1x.eap peap " + " 802-1x.phase2-auth mschapv2 " + " 802-1x.identity " + shellQuote(username);
        } else {
                ...
                StackLayout {
                    ...
                    ColumnLayout {
                        ...
                        PillField {
                            ...
                            onAccepted: {
                                if (pendingSavedUuid !== "")
                                    setSavedPskAndConnect(pendingSavedUuid, enteredPass);
                                else
                                    connectNew(targetSsid, enteredPass, enteredUser, targetIsEnterprise);
                            }
                        }
    ...
    component PillField: Rectangle {
        ...
        TextInput {
            ...
            onActiveFocusChanged: {
                if (activeFocus)
                    field.border.color = Qt.rgba(cGreen.r, cGreen.g, cGreen.b, 0.7);
                else
                    field.border.color = cBorder;
            }
        }
