...
Lib.Card {
    ...
    function cyclePerf() {
        ...
        var next = "balanced";
        if (current.includes("low"))
            next = "balanced";
        else if (current === "balanced")
            next = "balanced-performance";
        else if (current.includes("balanced-performance"))
            next = "performance";
        else if (current.includes("performance"))
            next = "low-power";
    ...
    function getPerfIcon() {
        ...
        if (s === "balanced")
            return "󰾅";
        if (s.includes("balanced-performance"))
            return "";
        if (s.includes("performance"))
            return "";
        return "󰾅";
    }
    ...
    function getPerfColor() {
        ...
        if (s.includes("performance"))
            return Colors.error;
        return (root.theme ? root.theme.textPrimary : Colors.on_background);
    }
    ...
    Lib.CommandPoll {
        ...
        parse: function (o) {
            return String(o).includes("do-not-disturb");
        }
    ...
    ColumnLayout {
        ...
        ColumnLayout {
            ...
            Lib.ExpressiveSlider {
                ...
                value: 50
                Layout.fillWidth: true
                accentColor: (root.theme && root.theme.isDarkMode !== undefined && !root.theme.isDarkMode) ? root.theme.accentSlider : Colors.primary
