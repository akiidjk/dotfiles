...
Rectangle {
    ...
    onIsPlayingChanged: {
        ...
        if (!root.pendingToggle)
            root.uiPlaying = root.isPlaying;
    }

    property bool recentlyPaused: root.hasPlayer && root.lastPlayingMs > 0 && ((root.nowMs - root.lastPlayingMs) < 60000)
    ...
    function syncMetadata(force) {
        ...
        if (force || nt !== root.title)
            root.title = nt;
        if (force || na !== root.artist)
            root.artist = na;

        // Update raw artUrl
        if (force || nu !== root.artUrl)
            root.artUrl = nu;
    }
    ...
    function makeTrackKey() {
        if (!root.player)
            return "";
        return (root.player.trackTitle || "") + "|" + (root.player.trackArtist || "") + "|" + (root.player.trackArtUrl || "");
    }
    ...
    function pickPosDiv(rawPos, lenSeconds, preferredDiv) {
        ...
        if (!isFinite(p) || p <= 0)
            return preferredDiv || 1000000;

        if (!(lenSeconds > 0))
            return pickTimeDiv(p);
    ...
    function resetTimeFromPlayer(forceKeyReset) {
        ...
        root.displayPos = (root.lenSec > 0) ? Math.max(0, Math.min(root.lenSec, ps)) : Math.max(0, ps);
    ...
    Timer {
        ...
        onTriggered: {
            ...
            root.displayPos = (root.lenSec > 0) ? Math.max(0, Math.min(root.lenSec, p)) : Math.max(0, p);
        }
    }
    ...
    function fmtLen() {
        return (root.lenSec > 0.5) ? root.fmt(root.lenSec) : "--:--";
    }
    ...
    Image {
        ...
        onStatusChanged: {
            ...
            artSample.grabToImage(function (res) {
                if (!res || !res.image || !res.image.pixelColor)
                    return;
                var img = res.image;
    ...
    Item {
        ...
        Rectangle {
            anchors.fill: parent
            color: (root.effectiveArtUrl !== "") ? Qt.rgba(0, 0, 0, 0.65) : Colors.surface_container
        }
        Rectangle {
            anchors.fill: parent
            color: root.bgTint
    ...
    MouseArea {
        ...
        onClicked: {
            Quickshell.execDetached(["bash", "-lc", "~/.config/quickshell/now_playing/now_playing"]);
            root.closeRequested();
        }
    }
    ...
    RowLayout {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Lib.WavyProgress {
                    ...
                    trackColor: Qt.rgba(Colors.on_surface.r, Colors.on_surface.g, Colors.on_surface.b, 0.15)
                    fps: 24
                    speed: 1.0
                    amplitude: 2.5
                    frequency: 0.15
        ...
        Rectangle {
            ...
            radius: 21
            color: Qt.rgba(root.accentColor.r, root.accentColor.g, root.accentColor.b, playArea.containsMouse ? 0.20 : 0.12)
            Behavior on color {
                ColorAnimation {
                    duration: 140
                }
