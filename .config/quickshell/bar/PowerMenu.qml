...
PanelWindow {
    ...
    FocusScope {
        ...
        function getConfirmText() {
            if (pendingCmd === "shutdown")
                return "Power Off?";
            if (pendingCmd === "reboot")
                return "Reboot System?";
            if (pendingCmd === "logout")
                return "Log Out?";
            if (pendingCmd === "lock")
                return "Lock Screen?";
            if (pendingCmd === "suspend")
                return "Suspend?";
            return "Are you sure?";
        }
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            color: (root.confirmIndex === 1 || isHovered) ? (root.isDestructive() ? theme.danger : theme.accent) : theme.tile
                            ...
                            TapHandler {
                                onTapped: root.run(root.pendingCmd)
                            }
                        }
                    }
                }
